...
export class Block {
    ...
    _ForwardKinematics(angles) {
        ...
        for (let i = 0; i < this._noOfJoints; i++) {
            const prev = math.squeeze(rotationMatrix.subset(math.index(i, math.range(0, 4), math.range(0, 4))));
            let transform;
            if ((i == 0) || ((this._noOfJoints == 6) && ((i == 3) || (i == 5)))) {
                transform = math.multiply(this._Rotate_Z(angles[i]), this._Translate_XYZ(0, 0, this._jointLengths[i]));

            } else {
                transform = math.multiply(this._Rotate_X(angles[i]), this._Translate_XYZ(0, 0, this._jointLengths[i]));
            }
            rotationMatrix.subset(math.index(i + 1, math.range(0, 4), math.range(0, 4)), math.multiply(prev, transform));
        }
        return rotationMatrix;
    }

    _DotProduct(v1, v2) {
        if (v1.length != v2.length) { return 0; } else {
            let val = 0;
    ...
    InverseKinematics_restricted(x_t, y_t, z_t, theta_deg = 0) {
        ...
        const x = x_t - this._jointLengths[3] * Math.cos(theta) * Math.cos(angles[0] - pi / 2);
